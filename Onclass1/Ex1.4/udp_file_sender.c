...
int main(int argc, char *argv[]) {
    ...
    // send filename
    bytes_sent = sendto(sock, "receive.txt", strlen("receive.txt"), 0, (struct sockaddr*)&server_addr, sizeof(server_addr));
    if (bytes_sent < 0) {
    ...
    while ((bytes_read = fread(buffer, 1, BUF_SIZE, fp)) > 0) {
        char ndsend[256];
        strcpy(ndsend, "receive.txt_");
        strcat(ndsend, buffer);
        bytes_sent = sendto(sock, ndsend, bytes_read + 12, 0, (struct sockaddr*)&server_addr, sizeof(server_addr));
        if (bytes_sent < 0) {
    ...
    bytes_sent = sendto(sock, "receive.txt_receive.txt", 23, 0, (struct sockaddr*)&server_addr, sizeof(server_addr));
    if (bytes_sent < 0) {
